...
class ModelTrainer():
	...
	def read_data(self, data_path):
		...
		delay = self.config.feature_size
		for text in open(data_path):
			text = self.vocab_encode(text)
			for start in range(len(text) - window - delay + 2):
				chunk = text[start : start + delay + window - 1]
				yield chunk
	...
	def get_batch(self, stream):
		...
		for element in stream:
			...
			label_batch.append(element[feature_size:feature_size+window]) 
